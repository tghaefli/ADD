...
inline xc_status_t xc_sg_gamma_s6_plbw_create(xc_iface_t **iface, void *config_table)
{
    // set up iface
    *iface = (xc_iface_t *) config_table;

#ifdef XC_DEBUG
    SG_GAMMA_S6_PLBW_Config *_config_table = config_table;
...
inline xc_status_t xc_sg_gamma_s6_plbw_release(xc_iface_t **iface) 
{
    // does nothing
    return XC_SUCCESS;
}

inline xc_status_t xc_sg_gamma_s6_plbw_open(xc_iface_t *iface)
{
    // does nothing
    return XC_SUCCESS;
}

inline xc_status_t xc_sg_gamma_s6_plbw_close(xc_iface_t *iface)
{
    // does nothing
    return XC_SUCCESS;
}

inline xc_status_t xc_sg_gamma_s6_plbw_read(xc_iface_t *iface, xc_r_addr_t addr, uint32_t *value)
{
    *value = *((uint32_t *) addr);
    return XC_SUCCESS;
}

inline xc_status_t xc_sg_gamma_s6_plbw_write(xc_iface_t *iface, xc_w_addr_t addr, const uint32_t value)
{
    *((uint32_t *) addr) = value;
    return XC_SUCCESS;
}
...
{
    SG_GAMMA_S6_PLBW_Config *_config_table = (SG_GAMMA_S6_PLBW_Config *) iface;

    if (strcmp("gamma_buffer", name) == 0) {
        *shmem = (void *) & _config_table->gamma_buffer;
    } else if (strcmp("gamma_b", name) == 0) {
        *shmem = (void *) & _config_table->gamma_b;
    } else if (strcmp("gamma_g", name) == 0) {
        *shmem = (void *) & _config_table->gamma_g;
    } else if (strcmp("gamma_r", name) == 0) {
        *shmem = (void *) & _config_table->gamma_r;
    }
