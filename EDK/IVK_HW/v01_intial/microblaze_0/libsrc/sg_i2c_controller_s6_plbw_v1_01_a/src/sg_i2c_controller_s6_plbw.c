...
inline xc_status_t xc_sg_i2c_controller_s6_plbw_create(xc_iface_t **iface, void *config_table)
{
    // set up iface
    *iface = (xc_iface_t *) config_table;

#ifdef XC_DEBUG
    SG_I2C_CONTROLLER_S6_PLBW_Config *_config_table = config_table;
...
inline xc_status_t xc_sg_i2c_controller_s6_plbw_release(xc_iface_t **iface) 
{
    // does nothing
    return XC_SUCCESS;
}

inline xc_status_t xc_sg_i2c_controller_s6_plbw_open(xc_iface_t *iface)
{
    // does nothing
    return XC_SUCCESS;
}

inline xc_status_t xc_sg_i2c_controller_s6_plbw_close(xc_iface_t *iface)
{
    // does nothing
    return XC_SUCCESS;
}

inline xc_status_t xc_sg_i2c_controller_s6_plbw_read(xc_iface_t *iface, xc_r_addr_t addr, uint32_t *value)
{
    *value = Xil_In32((uint32_t *) addr);
    return XC_SUCCESS;
}

inline xc_status_t xc_sg_i2c_controller_s6_plbw_write(xc_iface_t *iface, xc_w_addr_t addr, const uint32_t value)
{
...
{
    SG_I2C_CONTROLLER_S6_PLBW_Config *_config_table = (SG_I2C_CONTROLLER_S6_PLBW_Config *) iface;

    if (strcmp("gpio_out8", name) == 0) {
        *shmem = (void *) & _config_table->gpio_out8;
    } else if (strcmp("cmd_response", name) == 0) {
        *shmem = (void *) & _config_table->cmd_response;
    } else if (strcmp("cmd_request", name) == 0) {
        *shmem = (void *) & _config_table->cmd_request;
    }
