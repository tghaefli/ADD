...
struct struct_fmc_iic_xps_t
{
	Xuint32 CoreAddress;
};
typedef struct struct_fmc_iic_xps_t fmc_iic_xps_t;
...
int fmc_iic_xps_GpoRead ( fmc_iic_t *pIIC, Xuint32 *pGpioData );
int fmc_iic_xps_GpoWrite( fmc_iic_t *pIIC, Xuint32 GpioData );
int fmc_iic_xps_IicWrite( fmc_iic_t *pIIC, Xuint8 ChipAddress, Xuint8 RegAddress, 
                                           Xuint8 *pBuffer, Xuint8 ByteCount);
int fmc_iic_xps_IicRead ( fmc_iic_t *pIIC, Xuint8 ChipAddress, Xuint8 RegAddress, 
                                           Xuint8 *pBuffer, Xuint8 ByteCount);
int fmc_iic_xps_IicEWrite( fmc_iic_t *pIIC, Xuint8 ChipAddress, Xuint16 RegAddress, 
                                            Xuint8 *pBuffer, Xuint8 ByteCount);
int fmc_iic_xps_IicERead ( fmc_iic_t *pIIC, Xuint8 ChipAddress, Xuint16 RegAddress, 
                                            Xuint8 *pBuffer, Xuint8 ByteCount);
...
*
******************************************************************************/
int fmc_iic_xps_init( fmc_iic_t *pIIC, char szName[], Xuint32 CoreAddress )
{
   XStatus Status;
   //fmc_iic_xps_t *pContext = (fmc_iic_xps_t *)malloc( sizeof(fmc_iic_xps_t) );
   //if ( pContext == NULL )
   //{
   //   xil_printf("Failed to allocate context data for FMC-IIC-XPS implementation\n\r" );
   //   return 0;
   //}
   fmc_iic_xps_t *pContext = (fmc_iic_xps_t *) (pIIC->ContextBuffer);
   if ( sizeof(fmc_iic_xps_t) > FMC_IIC_CONTEXT_BUFFER_SIZE )
   {
      xil_printf("FMC_IIC_CONTEXT_BUFFER_SIZE is not large enough for fic_iic_xps_t structure (increase to %d)\n\r", sizeof(fmc_iic_xps_t) );
      return 0;
   }
   ...
   pIIC->fpGpoRead   = &fmc_iic_xps_GpoRead;
   pIIC->fpGpoWrite  = &fmc_iic_xps_GpoWrite;
   pIIC->fpIicRead   = &fmc_iic_xps_IicRead;
   pIIC->fpIicWrite  = &fmc_iic_xps_IicWrite;
   pIIC->fpIicERead  = &fmc_iic_xps_IicERead;
   pIIC->fpIicEWrite = &fmc_iic_xps_IicEWrite;
...
int fmc_iic_xps_GpoRead( fmc_iic_t *pIIC, Xuint32 *pGpioData )
{
   ...
   *pGpioData = XIo_In32((pContext->CoreAddress + 0x124));
...
int fmc_iic_xps_GpoWrite( fmc_iic_t *pIIC, Xuint32 GpioData )
{
   ...
   //xil_printf("[%s] XIo_Out32((pContext->CoreAddress + 0x124),data) => 0x%08X\n\r", pContext->szName, XIo_In32(pContext->CoreAddress + 0x124) );
   XIo_Out32((pContext->CoreAddress + 0x124),GpioData);
                                          ...
                                          Xuint8 *pBuffer, Xuint8 ByteCount)
{
  ...
  do
  {
    StatusReg = XIo_In8(pContext->CoreAddress + XIIC_SR_REG_OFFSET);
    //xil_printf("[%s] XIo_In8(pContext->CoreAddress + XIIC_SR_REG_OFFSET) => 0x%02X\n\r", pContext->szName, StatusReg );
