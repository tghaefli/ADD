...
proc generate {drv_handle} {
    puts "Generating Macros for sg_xsvi_fanout_plbw driver access ... "
    ...
    eval xdefine_include_file $drv_handle "xparameters.h" "SG_XSVI_FANOUT_PLBW" "NUM_INSTANCES" ${xparam_config_table}
    eval sg_xdefine_include_file $drv_handle "xparameters.h" "SG_XSVI_FANOUT_PLBW" ${addr_config_table}
    # generate sg_xsvi_fanout_plbw_g.c
    eval xdefine_config_file $drv_handle "sg_xsvi_fanout_plbw_g.c" "SG_XSVI_FANOUT_PLBW" ${config_table}
}
...
proc sg_xdefine_include_file {drv_handle file_name drv_string args} {
    ...
    foreach periph ${periphs} {
        ...
        puts $file_handle "#define XPAR_[string toupper [xget_hw_name ${periph}]]_XC_CREATE    xc_sg_xsvi_fanout_plbw_create"
        puts $file_handle "#define XPAR_[string toupper [xget_hw_name ${periph}]]_XC_RELEASE   xc_sg_xsvi_fanout_plbw_release"
        puts $file_handle "#define XPAR_[string toupper [xget_hw_name ${periph}]]_XC_OPEN      xc_sg_xsvi_fanout_plbw_open"
        puts $file_handle "#define XPAR_[string toupper [xget_hw_name ${periph}]]_XC_CLOSE     xc_sg_xsvi_fanout_plbw_close"
        puts $file_handle "#define XPAR_[string toupper [xget_hw_name ${periph}]]_XC_READ      xc_sg_xsvi_fanout_plbw_read"
        puts $file_handle "#define XPAR_[string toupper [xget_hw_name ${periph}]]_XC_WRITE     xc_sg_xsvi_fanout_plbw_write"
        puts $file_handle "#define XPAR_[string toupper [xget_hw_name ${periph}]]_XC_GET_SHMEM xc_sg_xsvi_fanout_plbw_getshmem"
