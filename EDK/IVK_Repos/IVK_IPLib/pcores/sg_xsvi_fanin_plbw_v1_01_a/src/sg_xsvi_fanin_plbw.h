...
#ifndef __XL_SG_XSVI_FANIN_PLBW_H__
#define __XL_SG_XSVI_FANIN_PLBW_H__
...
typedef struct {
    ...
    Xuint16  DeviceId;
    uint32_t  BaseAddr;
} SG_XSVI_FANIN_PLBW_Config;

extern SG_XSVI_FANIN_PLBW_Config SG_XSVI_FANIN_PLBW_ConfigTable[];

// forward declaration of low-level functions
xc_status_t xc_sg_xsvi_fanin_plbw_create(xc_iface_t **iface, void *config_table);
xc_status_t xc_sg_xsvi_fanin_plbw_release(xc_iface_t **iface) ;
xc_status_t xc_sg_xsvi_fanin_plbw_open(xc_iface_t *iface);
xc_status_t xc_sg_xsvi_fanin_plbw_close(xc_iface_t *iface);
xc_status_t xc_sg_xsvi_fanin_plbw_read(xc_iface_t *iface, xc_r_addr_t addr, uint32_t *value);
xc_status_t xc_sg_xsvi_fanin_plbw_write(xc_iface_t *iface, xc_w_addr_t addr, const uint32_t value);
