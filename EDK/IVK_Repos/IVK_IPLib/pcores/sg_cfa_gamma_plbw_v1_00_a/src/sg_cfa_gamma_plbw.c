...
inline xc_status_t xc_sg_cfa_gamma_plbw_create(xc_iface_t **iface, void *config_table)
{
    // set up iface
    *iface = (xc_iface_t *) config_table;

#ifdef XC_DEBUG
    SG_CFA_GAMMA_PLBW_Config *_config_table = config_table;
...
inline xc_status_t xc_sg_cfa_gamma_plbw_release(xc_iface_t **iface) 
{
    // does nothing
    return XC_SUCCESS;
}

inline xc_status_t xc_sg_cfa_gamma_plbw_open(xc_iface_t *iface)
{
    // does nothing
    return XC_SUCCESS;
}

inline xc_status_t xc_sg_cfa_gamma_plbw_close(xc_iface_t *iface)
{
    // does nothing
    return XC_SUCCESS;
}

inline xc_status_t xc_sg_cfa_gamma_plbw_read(xc_iface_t *iface, xc_r_addr_t addr, uint32_t *value)
{
    *value = Xil_In32((uint32_t *) addr);
    return XC_SUCCESS;
}

inline xc_status_t xc_sg_cfa_gamma_plbw_write(xc_iface_t *iface, xc_w_addr_t addr, const uint32_t value)
{
    Xil_Out32((uint32_t *) addr, value);
    return XC_SUCCESS;
}

xc_status_t xc_sg_cfa_gamma_plbw_getshmem(xc_iface_t *iface, const char *name, void **shmem)
{
    SG_CFA_GAMMA_PLBW_Config *_config_table = (SG_CFA_GAMMA_PLBW_Config *) iface;
